...
function renderTable(data) {
    ...
    data.forEach(player => {
        ...
        tr.innerHTML = `<td class="col-rank-es">#${player.spainRank}</td><td class="col-rank-eu">${player.found ? '#' + player.rank : '-'}</td><td class="col-player"><div class="player-row-content">${avatarHtml}<div class="player-info"><span class="player-name" style="${isSelected ? 'color: var(--hs-gold);' : ''}">${name}</span><span class="player-tag">#${tag}</span><div class="badges-wrapper">${badgesHtml}</div></div></div></td><td class="col-mmr">${ratingFormatted}</td>`;
